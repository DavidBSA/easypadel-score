...
type LeaderRow = {
  ...
  played: number; pointsFor: number; pointsAgainst: number; diff: number;
};
...
export default function PlayerPage() {
  ...
  const [setLog, setSetLog] = useState<{ gamesA: number; gamesB: number }[]>([]);
  ...
  useEffect(() => {
    ...
    es.onmessage = (e) => {
      try {
        ...
        startTransition(() => applySession(data));
      } catch { /* ignore */ }
    };
    es.onerror = () => {
      es.close();
  ...
  function changeName() {
    ...
    setSubmittedPointsA(null); setSubmittedPointsB(null);
  }

  // ── Complete a set in SINGLE live scoring ────────────────────────────────
  function completeSet() {
    const gA = parseInt(currentGamesA, 10);
    const gB = parseInt(currentGamesB, 10);
    if (isNaN(gA) || isNaN(gB) || gA === gB) return;
    setSetLog((prev) => [...prev, { gamesA: gA, gamesB: gB }]);
    if (gA > gB) setLiveSetsA((v) => v + 1);
    else setLiveSetsB((v) => v + 1);
    setCurrentGamesA("");
    setCurrentGamesB("");
  }
  ...
  function resetAllScoringState() {
    ...
    setFinalSetsA(0); setFinalSetsB(0);
  ...
  async function submitScore() {
    ...
    if (!deviceId) { setSubmitError("Device not registered — tap 'Change name' and re-select your name."); return; }
    ...
    if (isSingle) {
      // SINGLE format: submit sets won as pointsA / pointsB
      ...
      pA = scoreA; pB = scoreB;
      if (pA + pB !== total) {
        setSubmitError("Total must equal " + total + " pts. Currently " + (pA + pB) + " of " + total + " played.");
        setSubmitting(false); return;
      }
    }
    ...
    try {
      ...
      setSubmittedPointsA(pA);
      setSubmittedPointsB(pB);
  ...
  const myMatch = (selectedPlayer && session)
    ? (
        ...
        session.matches
          ...
          .sort((a, b) => b.queuePosition - a.queuePosition)[0]
      )
    : undefined;
  ...
  // ── SINGLE live scoring derived values ───────────────────────────────────
  const canCompleteSet = (() => {
    const gA = parseInt(currentGamesA, 10);
    const gB = parseInt(currentGamesB, 10);
    return !isNaN(gA) && !isNaN(gB) && gA !== gB;
  })();
  ...
  const leaderboard: LeaderRow[] = (() => {
    ...
    for (const p of session.players) {
      base.set(p.id, { playerId: p.id, name: p.name, played: 0, pointsFor: 0, pointsAgainst: 0, diff: 0 });
    }
    for (const m of session.matches) {
      if (m.status !== "COMPLETE" || m.pointsA === null || m.pointsB === null) continue;
      const pA = m.pointsA; const pB = m.pointsB;
      for (const pid of [m.teamAPlayer1, m.teamAPlayer2]) {
        const row = base.get(pid); if (row) { row.played++; row.pointsFor += pA; row.pointsAgainst += pB; }
      }
      for (const pid of [m.teamBPlayer1, m.teamBPlayer2]) {
        const row = base.get(pid); if (row) { row.played++; row.pointsFor += pB; row.pointsAgainst += pA; }
      }
    }
  ...
  const sessionComplete = !!(session && session.matches.length > 0 &&
    session.matches.every((m) => m.status === "COMPLETE"));
  ...
  const st: Record<string, React.CSSProperties> = {
    ...
    // SINGLE scoring specific
    setsDisplay: { display: "flex", justifyContent: "center", alignItems: "baseline", gap: 16, padding: "14px 0 10px" },
  ...
  const leaderboardPanel = showLeaderboard && (
    <>
      ...
      <div style={st.lbWrap}>
        <div style={st.lbHead}>
          <div style={st.lbCenter}>#</div>
          <div>Player</div>
          {!isMobile && <div style={st.lbRight}>Played</div>}
          <div style={st.lbRight}>Diff</div>
        </div>
        {leaderboard.map((row, idx) => {
          const isMe = row.playerId === selectedPlayer.id;
          const isTop3 = idx < 3 && row.played > 0;
          return (
            <div key={row.playerId} style={{
              ...st.lbRow,
              background: isMe ? "rgba(255,107,0,0.12)" : isTop3 ? "rgba(255,255,255,0.05)" : "rgba(255,255,255,0.02)",
              ...
              <div style={{ ...st.lbRight, fontSize: 13, fontWeight: 1100, color: row.diff > 0 ? GREEN : row.diff < 0 ? RED : WHITE }}>
        ...
        {completedCount === 0 && (
          <div style={{ padding: "12px 14px", fontSize: 12, opacity: 0.5, textAlign: "center" as const }}>No completed matches yet.</div>
        )}
      </div>
    </>
  );
    ...
    <>
      ...
      <div style={{ display: "grid", gap: 8 }}>
        {upcomingMatches.slice(0, 3).map((m, i) => {
          ...
          const partner = isMyTeamA
            ? nameById[m.teamAPlayer1 === selectedPlayer.id ? m.teamAPlayer2 : m.teamAPlayer1]
            : nameById[m.teamBPlayer1 === selectedPlayer.id ? m.teamBPlayer2 : m.teamBPlayer1];
          ...
          return (
            <div key={m.id} style={st.upcomingCard}>
              <div style={st.upcomingMatch}>
                <span style={{ color: ORANGE, fontWeight: 1000 }}>You &amp; {partner}</span>
        ...
        {upcomingMatches.length > 3 && (
          <div style={{ fontSize: 12, opacity: 0.45, textAlign: "center" as const, paddingTop: 4 }}>
  ...
  if (sessionComplete) {
    ...
    statusBlock = (
      <>
        <div style={{ borderRadius: 18, padding: 24, background: "rgba(255,107,0,0.08)", border: "1px solid rgba(255,107,0,0.30)", textAlign: "center" as const }}>
          ...
          {myRow && (
            <>
              <div style={{ fontSize: 36, fontWeight: 1200, marginTop: 12 }}>{rankLabel}</div>
              <div style={{ fontSize: 15, fontWeight: 1000, marginTop: 6 }}>
                {myRow.pointsFor} pts · {myRow.diff > 0 ? "+" + myRow.diff : myRow.diff} diff
              </div>
            </>
          )}
        </div>
        ...
        <div style={st.lbWrap}>
          ...
          {leaderboard.map((row, idx) => {
            ...
            return (
              <div key={row.playerId} style={{
                ...st.lbRow,
                background: isMe ? "rgba(255,107,0,0.12)" : idx < 3 ? "rgba(255,255,255,0.05)" : "rgba(255,255,255,0.02)",
                borderTop: "1px solid rgba(255,255,255,0.05)",
              }}>
                <div style={{ ...st.lbCenter, fontSize: 14, fontWeight: 1100, color: idx === 0 ? ORANGE : WHITE }}>{idx + 1}</div>
                <div style={{ fontSize: 14, fontWeight: isMe ? 1100 : 900, color: isMe ? ORANGE : WHITE }}>{row.name}{isMe ? " *" : ""}</div>
                {!isMobile && <div style={{ ...st.lbRight, fontSize: 13, opacity: 0.7 }}>{row.played}</div>}
                <div style={{ ...st.lbRight, fontSize: 13, fontWeight: 1100, color: row.diff > 0 ? GREEN : row.diff < 0 ? RED : WHITE }}>
                  {row.diff > 0 ? "+" + row.diff : row.diff}
                </div>
              </div>
    ...
    const pA = myMatch.pointsA ?? 0;
    const pB = myMatch.pointsB ?? 0;
    ...
    if (upcomingMatches.length > 0) {
      statusBlock = (
        <>
          <div style={statusBoxStyle(ORANGE)}>
            <div style={{ fontSize: 40, marginBottom: 10 }}>⏳</div>
            <div style={{ fontWeight: 1000, fontSize: 20 }}>Waiting for next court</div>
            <div style={{ opacity: 0.6, marginTop: 8, fontSize: 14, lineHeight: 1.5 }}>
              Your next match is queued — you'll be called to a court shortly.
            </div>
          </div>
          <div style={st.resultPill}>
            <div style={{ fontSize: 13, fontWeight: 900, opacity: 0.7 }}>Last result</div>
            <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
              <span style={{ fontSize: 22, fontWeight: 1200, color: myTeam === "A" ? ORANGE : WARM_WHITE }}>{pA}</span>
              <span style={{ opacity: 0.35, fontSize: 16 }}>–</span>
              <span style={{ fontSize: 22, fontWeight: 1200, color: myTeam === "B" ? ORANGE : WARM_WHITE }}>{pB}</span>
            </div>
            <div style={{ fontSize: 12, fontWeight: 1000, color: won ? GREEN : mine === theirs ? WARM_WHITE : RED }}>
              {won ? "Won" : mine === theirs ? "Draw" : "Lost"}
            </div>
          </div>
          {upcomingBlock}
        </>
      ...
      statusBlock = (
        <>
          ...
          <div style={{ marginTop: 12, borderRadius: 14, padding: "12px 16px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", fontSize: 13, fontWeight: 900, color: WARM_WHITE, opacity: 0.7, textAlign: "center" as const }}>
            No more matches queued yet — check back shortly.
          </div>
        </>
      );
    }
    ...
    const a1 = nameById[myMatch.teamAPlayer1];
    const a2 = nameById[myMatch.teamAPlayer2];
    const b1 = nameById[myMatch.teamBPlayer1];
    const b2 = nameById[myMatch.teamBPlayer2];

    if (submitResult === "CONFIRMED" && submittedPointsA !== null && submittedPointsB !== null) {
      const pA = submittedPointsA;
      const pB = submittedPointsB;
      const mine = myTeam === "A" ? pA : pB;
      const theirs = myTeam === "A" ? pB : pA;
      statusBlock = (
        <>
          <div style={statusBoxStyle(GREEN)}>
            ...
            <div style={{ opacity: 0.6, fontSize: 13 }}>
              {mine > theirs ? "You won " + mine + "–" + theirs : mine === theirs ? "Draw!" : "You scored " + mine}
            </div>
          </div>
          {upcomingBlock}
        </>
      ...
      // ── Live scoring: cap total at match pointsPerMatch (Americano only) ──
      const liveTotalPlayed = scoreA + scoreB;
      const liveAtMax = liveTotalPlayed >= total;
      const canAddA = !liveAtMax;
      const canAddB = !liveAtMax;
        ...
        <>
          ...
          {isSingle ? (
                  ...
                  <div style={st.grid2}>
                    <div>
                      ...
                      <div style={st.scoreRow}>
                        ...
                        <button style={st.stepBtn} onClick={() => setFinalSetsA((v) => v + 1)}>+</button>
                      </div>
                    </div>
                    <div>
                      ...
                      <div style={st.scoreRow}>
                        ...
                        <button style={st.stepBtn} onClick={() => setFinalSetsB((v) => v + 1)}>+</button>
                      </div>
                    </div>
                  </div>
                  <div style={st.hint}>Enter the final sets won by each team.</div>
                </>
              ) : (
                // ── Live scoring: game inputs + set completion ────────────
                <>
                  {/* Running sets total */}
                  ...
                  {setLog.length > 0 && (
                    <div style={{ marginBottom: 12, display: "grid", gap: 2 }}>
                      {setLog.map((s, i) => (
                        <div key={i} style={st.setLogItem}>
                          Set {i + 1}: {s.gamesA}–{s.gamesB} · {s.gamesA > s.gamesB ? (myTeam === "A" ? "You won" : "Team A won") : (myTeam === "B" ? "You won" : "Team B won")}
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  <div style={st.setCurrentLabel}>Set {setLog.length + 1} — games</div>
                  <div style={st.grid2}>
                    <div>
                      <div style={st.teamLabel}>Team A{myTeam === "A" ? " · You" : ""}</div>
                      <input
                        ...
                        placeholder="0"
                        value={currentGamesA}
                        onChange={(e) => setCurrentGamesA(e.target.value.replace(/[^\d]/g, ""))}
                      />
                    </div>
                    <div>
                      <div style={st.teamLabel}>Team B{myTeam === "B" ? " · You" : ""}</div>
                      <input
                        ...
                        placeholder="0"
                        value={currentGamesB}
                        onChange={(e) => setCurrentGamesB(e.target.value.replace(/[^\d]/g, ""))}
                      />
                    </div>
                  </div>
                  ...
                  {/* Complete Set button */}
                  <button
                    style={{ ...st.secondaryBtn, opacity: canCompleteSet ? 1 : 0.35 }}
                    onClick={completeSet}
                    disabled={!canCompleteSet}
                  >
                    ✓ Complete Set {setLog.length + 1}
                  </button>

                  <div style={st.hint}>
                    Update games as the set progresses. Tap "Complete Set" when the set finishes to log it and move to the next.
                  </div>
                </>
              )}
            ...
            <>
                ...
                <>
                  <div style={st.grid2}>
                    <div style={{ textAlign: "center" as const }}>
                      <div style={st.teamLabel}>Team A{myTeam === "A" ? " · You" : ""}</div>
                      <div style={st.scoreRow}>
                        <button style={{ ...st.stepBtn, opacity: canAddA ? 1 : 0.35 }} onClick={() => { if (canAddA) setScoreA((v) => v + 1); }} disabled={!canAddA}>+</button>
                    ...
                    <div style={{ textAlign: "center" as const }}>
                      <div style={st.teamLabel}>Team B{myTeam === "B" ? " · You" : ""}</div>
                      <div style={st.scoreRow}>
                        <button style={{ ...st.stepBtn, opacity: canAddB ? 1 : 0.35 }} onClick={() => { if (canAddB) setScoreB((v) => v + 1); }} disabled={!canAddB}>+</button>
                  ...
                  <div style={st.hint}>
                    {liveTotalPlayed} of {total} pts played.
                    {liveAtMax ? " Ready to submit." : " — " + (total - liveTotalPlayed) + " pts remaining."}
                  </div>
              ...
              <button
                style={{ ...st.primaryBtn, opacity: (submitting || (scoringMode === "live" && !liveAtMax)) ? 0.5 : 1 }}
                onClick={submitScore}
                disabled={submitting || (scoringMode === "live" && !liveAtMax)}
              >
      ...
      <div style={st.card}>
        <div style={st.row}>
          <div>
            ...
            <div style={st.sub}>Session {code} · {session ? formatLabel(session.format) : ""}</div>
          </div>
          {!sessionComplete && (
            <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
              <button
                style={showLeaderboard ? st.btnActive : st.btn}
